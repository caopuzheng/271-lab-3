...
> Based on the residuals analysis, the State-level Fixed Effects model will generate more reliable estimates. 
...
> While we don't think it is appropriate to use the random effect model, for information only, we included the random effect model in the comparison below.
...
> Since the null hypothesis for the Hausman test is rejected, the last assumption is not met. Thus we should not use random effect model to estimate the coefficients since the estimates will not likely to be consistent.

> Random effects model is also not likely to be efficient, which means its variance would not be minimum and the standard error estimates could be biased. 
