...
> We plotted total_fatalities_rate by state below to understand the trend over the years. 
...
> The model residuals improved over the prelimary linear model version based on the plots. However, the residual mean still has a lot of fluctuations and not around zero. The Normal Q-Q plot is closer to the normal distribution, better than the preliminary linear model. But serial correlation is still a major problem for the model residuals, based on the Breusch-Godfrey test and the ACF plot. Serial correlation makes standard error estimates and statistical inferences not reliable. 
...
```{r}
df <- pdata.frame(df, index = c('year_of_observation','state'))
...
> In this State-level fixed effects model, the estimated coefficient on blood alcohol limit 0.08 is -0.0048883, much less negative than the estimated coefficient of -0.045924 in the expanded linear model (mod.lm2). 

> Additionally, the expanded linear model (mod.lm2) showed that this variable is statistically significant, but the State-level fixed effects model shows taht this variable's coefficient is not significantly different from zero. 

> Its interpretation is that changing the blood alcohol limit from 0.10% to 0.08% causes traffic fatalities rate to decrease marginally by 0.49%, which is not significant from statistical and practical perspectives. 
...
> The estimated coefficient on *per se laws* is -0.0554823 and significantly different from zero at 0%. In the prior expanded linear model (mod.lm2), this variable has a coefficient of -0.022 and was not statistically significant.
...
> The estimated coefficient on *primary seatbelt laws* is -0.041 and has strong evidence that it is difference from zero at 0.1%. The prior expanded model (mod.lm2) noted that this variable was not statistically significant, which was not aligned with our experience.

> Its interpretation is that chaning seatbelt law requirement from none to primary will cause traffic fatalities rate to decrease by 4.1%, which is significant both statistically and practically. This effect makes much sense than the effect estimated by the expanded model. 
...
> For the State-level Fixed Effect models, as we extend the model to include the entire fixed effects, it mitigated the omitted variable bias issues and improved the model performance over the linear model versions.

> The model residuals have a conditional mean around zero, with a normal distribution. Since this model uses the within setting to fit a fixed effect model at the state level, the effect of unobserved individual heterogeneity is eliminated. The conditinoal mean of zero assumption is met. 

> Variance fluctulations are much improved compared to the expanded model. The ACF plot for the model residuals shows autocorrelation for the first few lags but decays quickly (this is much improved compared to the significant but non-decaying autocorrelation in the expanded model). We used the robust standard errors to account for the group heteroskedasticity and serial correlation by allowing correlation across time in groups.  

> Based on the residuals analysis, the State-level Fixed Effects model will generate more reliable estimates. R2 increased from 0.67 to 0.73, as fixed effects are included in the model equation. 

> Also, the parameter estimates' sign and magnitude in the Fixed Effects model make much sense than the expanded model, based on our EDA work and our experience. 
...
# While we don't think it is appropriate to use the random effect model, for information only, we included the random effect model in the comparison for information
...
> Since the null hypothesis for the Hausman test is rejected, the last assumption is not met. Thus we should not use random effect model to estimate the coefficients since it will be bias. 
