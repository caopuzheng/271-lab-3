...
- What, if any, are the limitation of this model. In answering this, please consider **at least**: 
    ...
    > TODO: let's do this. Not sure what the difference is between parameter estimates and uncertainty estiamtes. 
    > We can note on R^2.
...
ggplot() +
  geom_line(
    ...
    aes(x = year, y = total, color = "new data")
  ) +
  geom_line(
    data = df_annual,
    aes(x = year, y = total, color = "Data from the pa")
  ) +
  labs(
    title = "US Vehicle Miles Traveled (billions)",
    subtitle = paste0(
      "Data from Bloomberg vs assignment\nAvg difference by year: ",
      avg_diff, "%"
    ),
    x = "Year",
...
vehicle_miles_month %>%
  ...
  group_by(year, month) %>%
  summarise(
    vehicle_miles = sum(vehicle_miles)
  ) %>%
  ungroup() %>%
  mutate(
    vehicle_miles = vehicle_miles / 1000000
  ) %>%
  spread(year, vehicle_miles) %>%
  mutate(
    pct_change = (2020 - 2018) / 2018
  ) %>%
  arrange(month)
  ```
