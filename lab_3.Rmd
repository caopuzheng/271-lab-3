...
> We decided to not show the correlation matrix for the discrete variables, because the correlation matrix is not a good way to visualize the relationship between discrete variables and continuous variables.
...
- What, if any, are the limitation of this model. In answering this, please consider **at least**: 
    ...
    > We can note on R^2
    > there is seriel correlation b/c the data is not IID (all from the same state).
    > beta coefficent is too small. Should use a robust standard error.
```{r}
